...
namespace UIExtention {
	public class TriangulationViewerWindow : EditorWindow {

		[SerializeField]
		private GradationMaterial m_Material = null;
		private GradationMaterial material {get{
			if (m_Material == null) {
				m_Material = new GradationMaterial();
			}
			return m_Material;
		}}
		...
		[MenuItem("UIExtention/TriangulationViewerWindow")]
		public static TriangulationViewerWindow Instantiate() {
			var result = EditorWindow.GetWindow<TriangulationViewerWindow>();
			result.Show();
			return result;
		}
		...
		protected virtual void OnEnable() {
			...
			titleContent = new GUIContent("Triangulation Test");
		...
		private void GradationMap(Rect r) {
			if (Event.current.type == EventType.Repaint) {
				DrawTarget(r);
				DrawSnapLines(r);
				DrawTriangulationLines(r);
			}

			for(int i = 0, i_max = material.keys.Count; i < i_max; ++i) {
				var isFocus = (m_Focus != null) && m_Focus.Any(x=>x == i);
				EditorGUI.BeginChangeCheck();
				var key = GradationMaker(r, material.keys[i].position, material.keys[i].color, isFocus, k_MakerRadius, GetSnapPositions(r, i), i);
				if (EditorGUI.EndChangeCheck()) {
		...
		private void DrawTriangulationLines(Rect r) {
			if (!m_Snap) {
				return;
			}
			...
			var keys = material.keys.Select(x=>x.position)
									.Select(x=>{
										x.x = Mathf.LerpUnclamped(r.xMin, r.xMax, x.x);
										x.y = Mathf.LerpUnclamped(r.yMin, r.yMax, x.y);
										return x;
									})
									.ToList();
			var triangulationIndex = Utility.Triangulation(keys);

			for (int i = 2, iMax = triangulationIndex.Count; i < iMax; i+=3) {
				Handles.DrawLine(keys[triangulationIndex[i-2]], keys[triangulationIndex[i-1]]);
				Handles.DrawLine(keys[triangulationIndex[i-1]], keys[triangulationIndex[i-0]]);
				Handles.DrawLine(keys[triangulationIndex[i-0]], keys[triangulationIndex[i-2]]);
			}
		...
		private static Vector2 GradationMaker(Rect r, Vector2 value, Color color, bool isFocus, float radius, IEnumerable<Vector2> snapPositions, int index) {
			int controlID = GUIUtility.GetControlID(FocusType.Passive);
			return GradationMaker(controlID, r, value, color, isFocus, radius, snapPositions, index);
		}
		private static Vector2 GradationMaker(int controlID, Rect r, Vector2 value, Color color, bool isFocus, float radius, IEnumerable<Vector2> snapPositions, int index) {
			...
			case EventType.Repaint:
				...
				Handles.Label(position + new Vector2(-0.5f, -0.5f) * GUI.skin.font.fontSize, index.ToString());
